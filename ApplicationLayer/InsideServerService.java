...
public class InsideServerService implements Service{
	int msgCode;
	boolean check;
	Vector msgContent;
	DatabaseManager dbMng;
	public InsideServerService(DatabaseManager dbMng)
	{
		this.dbMng =dbMng;
	}
	public Message serveService(Message receivedMsg, String ipAddress)
	{
		...
		switch (msgCode)
		{
            ...
            case Message.PROCTOR_LOG_OUT:
        			...
        			sendMsg.setContent(msgContent);
        			break;
            case Message.SET_AUTHORIZE:
            		System.out.println("Setting Authorized");
            		setAuthorized((ExamRecord)msgContent.get(0));
            		msgContent = null;
            		sendMsg.setContent(msgContent);
		}
...
//		}
	...
	private void proctorLogout(User user)
	{
     ...
     private void setAuthorized(ExamRecord examRecord)
     {
    	 check = this.dbMng.setAuthorized(examRecord);
    	 if (!check) msgCode =-1;
